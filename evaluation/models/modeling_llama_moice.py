...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # import pdb
        # pdb.set_trace()
        attn_weights = torch.matmul(query_states.permute(0,2,1,3), key_states.permute(0,2,3,1)) / math.sqrt(self.head_dim) #
        attn_weights = attn_weights + attention_mask.repeat(self.expert_nums*bsz,1,1,1)# expert_nums*bsz head q_len kv_seq_len

        # upcast attention to fp32
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_output = torch.matmul(attn_weights, value_states.permute(0,2,1,3)) # expert_nums*bsz head q_len dim

        attn_output = routing_weights.unsqueeze(-1) * attn_output.permute(0,2,1,3).view(self.expert_nums,bsz,q_len,self.num_heads,self.head_dim)
