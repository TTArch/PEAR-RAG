...
# from easy_transformer.modeling_milm import MiLMForCausalLM
# from easy_transformer.tokenization_milm import MiTokenizer
# from easy_transformer.configuration_milm import MiConfig
...
from easy_transformer.components_llama import *
...
class EasyTransformer(HookedRootModule):
    ...
    def __init__(
        ...
        if self.cfg.tokenizer_name is not None:
            ...
            if 'milm' in self.cfg.tokenizer_name:
                self.tokenizer = MiTokenizer(os.path.join(self.cfg.tokenizer_name, "mitokenizer.model")) #.tokenizer
            else:
                # If we have a tokenizer name, we can load it from HuggingFace
                self.tokenizer = AutoTokenizer.from_pretrained(self.cfg.tokenizer_name)
            self.tokenizer.pad_token = self.tokenizer.eos_token
            self.tokenizer.name_or_path = self.cfg.tokenizer_name
        else:
    ...
    def forward(
        ...
        gpu_num = torch.cuda.device_count() - 2
        # device_id = '1'
        for i, block in enumerate(self.blocks):
            # Note that each block includes skip connections, so we don't need
            # residual + block(residual)
            device_id = int(i / self.cfg.n_layers * gpu_num) + 1
            block = block.cuda(device_id) #to(f'cuda:{device_id}')
            residual = residual.cuda(device_id) #.to(f'cuda:{device_id}')
        ...
        **model_kwargs,
        ...
        if hf_model is None:
            if checkpoint is not None:
                ...
                if 'milm' in hf_model_name:
                    hf_model =  MiLMForCausalLM.from_pretrained(hf_model_name)
                else:
                    hf_model = AutoModelForCausalLM.from_pretrained(hf_model_name)
        ...
        if 'milm' in cfg.tokenizer_name:
            tokenizer = MiTokenizer(os.path.join(cfg.tokenizer_name, "mitokenizer.model"))
            #tokenizer = mitokenizer.tokenizer
            #tokenizer.eos_token = mitokenizer.eos_token 
        else:
            tokenizer = AutoTokenizer.from_pretrained(cfg.tokenizer_name)
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.name_or_path = cfg.tokenizer_name
        ori = True
        ori = False
        if not ori: 
            ...
            elif model_family == "milm":
                state_dict = weight_conversion.convert_milm_weights(hf_model, model.cfg)
    ...
    def convert_hf_config(cls, hf_config, model_family):
        ...
        if model_family == "neo":
            ...
            cfg_dict = {
                "d_model": hf_config.hidden_size,
                "d_head": hf_config.hidden_size // hf_config.decoder_attention_heads,
                "n_heads": hf_config.decoder_attention_heads,
                "d_mlp": hf_config.decoder_embed_dim,
                "n_layers": hf_config.decoder_layers,
                "n_ctx": hf_config.n_positions,
                "eps": 1e-6,
                "d_vocab": hf_config.num_embeddings,
                "act_fn": hf_config.mlp_name,
                "use_attn_scale": True,
                "use_local_attn": False,
                "scale_attn_by_inverse_layer_idx": False,
                "positional_embedding_type": "rotary",
                "ffn_dim_multiplier": hf_config.ffn_dim_multiplier,
                "multiple_of":hf_config.multiple_of,
                "tp_size":hf_config.tp_size,
                "torch_dtype":hf_config.torch_dtype,
                "rotary_dim": hf_config.hidden_size // hf_config.decoder_attention_heads,
            ...
            cfg_dict = {
                ...
                "padding_idx": hf_config.pad_token_id,
                "ffn_dim_multiplier": hf_config.intermediate_size,
                "tp_size":hf_config.pretraining_tp,
            }
        else:
    ...
    def load_and_process_state_dict(
        ...
        move_dict_to_device: bool = False,
    ):
    ...
    def center_writing_weights(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for l in range(self.cfg.n_layers):
            ...
            state_dict[f"blocks.{l}.mlp.w1"] = state_dict[
    ...
    def generate(
        ...
        for index in tqdm.tqdm(range(max_new_tokens)):
            ...
            if stop_at_eos:
                ...
                sampled_tokens = sampled_tokens.to(tokens)
