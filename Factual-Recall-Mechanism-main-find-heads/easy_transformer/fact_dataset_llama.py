...
def get_name_idxs(prompts, tokenizer, idx_types=["N, R"], prepend_bos=False):
    ...
    for prompt in prompts:
        t = prompt["text"].split(" ")
        toks = tokenizer.tokenize(" ".join(t))
         
        for idx_type in idx_types:
            if idx_type == 'R':
                idx = len(toks) - toks[-1::-1].index(tokenizer.tokenize(" " + prompt[idx_type])[1]) - 1
            elif idx_type == 'N':
                idx = toks.index(tokenizer.tokenize(" " + prompt[idx_type])[1])
                idx = int(len(toks) / 2 + idx)
            elif idx_type == 'ICL1L':
                idx = 5
            elif idx_type == 'ICL2L':
                idx = 12
            ...
            name_idx_dict[idx_type].append(idx + 1)
